  ...
  return <main className="page-shell">
    ...
    <div className="content-stack">
      ...
      {selectedVehicle && (permissions.has("administration.vehicle.maintenance.manage")||permissions.has("administration.vehicle.fuel.record")) ? <section className="organization-grid">
        {permissions.has("administration.vehicle.maintenance.manage") ? <article className="panel"><div className="panel-heading"><div><span className="eyebrow dark">Bakım ve servis</span><h2>Bakım geçmişi</h2><p>Maliyetleri ve sonraki bakım planını izleyin.</p></div><strong>{maintenance.length}</strong></div><OperationDisclosure title="Yeni bakım kaydı" description="Servis, maliyet ve sonraki bakım planını birlikte kaydedin."><form className="stack" onSubmit={e => submit(e, `/api/v1/administration/vehicles/${selectedVehicle.id}/maintenance`, fd => ({ odometerKm: Number(fd.get("km")), occurredAt: fd.get("occurredAt"), maintenanceType: fd.get("type"), description: fd.get("description"), cost: Number(fd.get("cost")), currency: fd.get("currency"), serviceDate: fd.get("serviceDate"), nextDueDate: fd.get("nextDate") || null, nextDueOdometerKm: Number(fd.get("nextKm")) || null, vendor: fd.get("vendor") || null }), "Bakım/servis kaydı oluşturuldu.")}><label className="field-label">Kilometre<input name="km" type="number" defaultValue={selectedVehicle.currentOdometerKm ?? 0} required/></label><label className="field-label">Kayıt zamanı<input name="occurredAt" type="datetime-local" defaultValue={nowLocal()} required/></label><label className="field-label">Bakım türü<input name="type" required/></label><label className="field-label">Açıklama<input name="description" required/></label><label className="field-label">Maliyet<input name="cost" type="number" min="0" step="0.01" defaultValue="0" required/></label><label className="field-label">Para birimi<input name="currency" defaultValue="TRY" maxLength={3} required/></label><label className="field-label">Servis tarihi<input name="serviceDate" type="date" defaultValue={today()} required/></label><label className="field-label">Sonraki tarih<input name="nextDate" type="date"/></label><label className="field-label">Sonraki kilometre<input name="nextKm" type="number"/></label><label className="field-label">Servis / tedarikçi<input name="vendor"/></label><button className="primary-button" disabled={busy}>Bakımı kaydet</button></form></OperationDisclosure><div className="table-wrap responsive-table-wrap" role="region" aria-label="Araç bakım geçmişi" tabIndex={0}><table className="data-table responsive-table"><thead><tr><th>Tarih</th><th>KM</th><th>Tür</th><th>Maliyet</th><th>Sonraki</th></tr></thead><tbody>{maintenance.length?maintenance.map(x => <tr key={x.id}><td data-label="Tarih">{formatDate(x.serviceDate)}</td><td data-label="KM">{x.odometerKm.toLocaleString("tr-TR")}</td><td data-label="Tür">{x.maintenanceType}<small>{x.vendor ?? "Tedarikçi yok"}</small></td><td data-label="Maliyet">{x.cost.toLocaleString("tr-TR",{minimumFractionDigits:2})} {x.currency}</td><td data-label="Sonraki">{formatDate(x.nextDueDate)}<small>{x.nextDueOdometerKm?`${x.nextDueOdometerKm.toLocaleString("tr-TR")} km`:"Kilometre planı yok"}</small></td></tr>):<tr><td className="empty-row" colSpan={5}>Henüz bakım kaydı yok.</td></tr>}</tbody></table></div></article> : null}
        {permissions.has("administration.vehicle.fuel.record") ? <article className="panel"><div className="panel-heading"><div><span className="eyebrow dark">Yakıt</span><h2>Yakıt geçmişi</h2><p>Litre, maliyet ve kilometre kayıtlarını izleyin.</p></div><strong>{fuel.length}</strong></div><OperationDisclosure title="Yeni yakıt kaydı" description="Yakıt miktarı, toplam maliyet ve kilometre bilgisini girin."><form className="stack" onSubmit={e => submit(e, `/api/v1/administration/vehicles/${selectedVehicle.id}/fuel`, fd => ({ odometerKm: Number(fd.get("km")), fueledAt: fd.get("fueledAt"), liters: Number(fd.get("liters")), totalCost: Number(fd.get("cost")), currency: fd.get("currency"), station: fd.get("station") || null, source: "MANUAL", externalEventId: null }), "Yakıt kaydı oluşturuldu.")}><label className="field-label">Kilometre<input name="km" type="number" defaultValue={selectedVehicle.currentOdometerKm ?? 0} required/></label><label className="field-label">Yakıt zamanı<input name="fueledAt" type="datetime-local" defaultValue={nowLocal()} required/></label><label className="field-label">Litre<input name="liters" type="number" min="0.001" step="0.001" required/></label><label className="field-label">Toplam maliyet<input name="cost" type="number" min="0" step="0.01" required/></label><label className="field-label">Para birimi<input name="currency" defaultValue="TRY" maxLength={3} required/></label><label className="field-label">İstasyon<input name="station"/></label><button className="primary-button" disabled={busy}>Yakıtı kaydet</button></form></OperationDisclosure><div className="table-wrap responsive-table-wrap" role="region" aria-label="Araç yakıt geçmişi" tabIndex={0}><table className="data-table responsive-table"><thead><tr><th>Zaman</th><th>KM</th><th>Litre</th><th>Maliyet</th><th>İstasyon</th></tr></thead><tbody>{fuel.length?fuel.map(x => <tr key={x.id}><td data-label="Zaman">{new Date(x.fueledAt).toLocaleString("tr-TR")}</td><td data-label="KM">{x.odometerKm.toLocaleString("tr-TR")}</td><td data-label="Litre">{x.liters.toLocaleString("tr-TR")}</td><td data-label="Maliyet"><strong>{x.totalCost.toLocaleString("tr-TR",{minimumFractionDigits:2})} {x.currency}</strong></td><td data-label="İstasyon">{x.station ?? "—"}</td></tr>):<tr><td className="empty-row" colSpan={5}>Henüz yakıt kaydı yok.</td></tr>}</tbody></table></div></article> : null}
      </section> : null}
    </div>
    {dialog}
  </main>;
