...
export default function PayrollPage() {
  ...
  return <main className="page-shell">
    ...
    {permissions.has("payroll.compensation.view") ? <section className="panel">
      ...
      {permissions.has("payroll.compensation.manage") && employeeId ? <OperationDisclosure title="Yeni ücret dönemi ekle" description="Tarih aralıkları çakışmamalı; kayıtlar geçmişe dönük izlenir."><form className="inline-form" onSubmit={createCompensation}><label className="field-label">Başlangıç<input name="validFrom" type="date" defaultValue={firstDayOfCurrentMonth()} required/></label><label className="field-label">Bitiş (hariç)<input name="validUntilExclusive" type="date"/></label><label className="field-label">Aylık baz ücret<input name="monthlyBaseSalary" type="number" min="0.01" step="0.01" required/></label><label className="field-label">Döviz<input name="currency" defaultValue={selectedCompany?.defaultCurrency ?? "TRY"} maxLength={3} required/></label><label className="field-label">FM çarpanı<input name="overtimeMultiplier" type="number" min="1" max="5" step="0.01" defaultValue="1.5" required/></label><button className="primary-button" disabled={busy || !employeeId}>{busy ? "Kaydediliyor…" : "Ücret ekle"}</button></form></OperationDisclosure> : null}
      <div className="table-wrap responsive-table-wrap" role="region" aria-label="Ücret geçmişi" tabIndex={0}><table className="data-table responsive-table"><thead><tr><th>Geçerlilik dönemi</th><th>Baz ücret</th><th>FM çarpanı</th></tr></thead><tbody>{compensations.map(x => <tr key={x.id}><td data-label="Geçerlilik">{formatDate(x.validFrom)} → {x.validUntilExclusive ? formatDate(x.validUntilExclusive) : "Devam ediyor"}</td><td data-label="Baz ücret"><strong>{formatMoney(x.monthlyBaseSalary, x.currency)}</strong></td><td data-label="FM çarpanı">{x.overtimeMultiplier.toLocaleString("tr-TR")}×</td></tr>)}{compensations.length === 0 ? <tr><td className="empty-row" colSpan={3}>{employeeId ? "Seçili personel için ücret tanımı yok." : "Ücret geçmişini görüntülemek için personel seçin."}</td></tr> : null}</tbody></table></div>
    </section> : null}
    ...
    <section className="panel">
      ...
      {permissions.has("payroll.period.manage") ? <OperationDisclosure title="Yeni dönem veya revizyon oluştur" description="Aynı ay yeniden oluşturulursa yeni bir revizyon açılır."><form className="inline-form" onSubmit={createPeriod}><label className="field-label">Yıl<input name="year" type="number" min="2000" max="2200" defaultValue={new Date().getFullYear()} required/></label><label className="field-label">Ay<input name="month" type="number" min="1" max="12" defaultValue={new Date().getMonth() + 1} required/></label><button className="primary-button" disabled={busy || !companyId}>Dönem oluştur</button></form></OperationDisclosure> : null}
      <div className="table-wrap responsive-table-wrap" role="region" aria-label="Bordro dönemleri" tabIndex={0}><table className="data-table responsive-table"><thead><tr><th>Dönem</th><th>Revizyon</th><th>Durum</th><th>Versiyon</th><th>İşlem</th></tr></thead><tbody>{periods.map(x => <tr key={x.id}><td data-label="Dönem"><strong>{monthLabel(x.year, x.month)}</strong></td><td data-label="Revizyon">Rev. {x.revision}</td><td data-label="Durum"><span className={`status-badge ${payrollStatusClass(x.status)}`}>{payrollStatusLabel(x.status)}</span></td><td data-label="Versiyon">v{x.version}</td><td data-label="İşlem"><button className="table-button" type="button" disabled={selectedPeriodId === x.id} onClick={() => void selectPeriod(x.id)}>{selectedPeriodId === x.id ? "Seçili" : "Dönemi seç"}</button></td></tr>)}{periods.length === 0 ? <tr><td className="empty-row" colSpan={5}>Bu şirket için bordro dönemi bulunmuyor.</td></tr> : null}</tbody></table></div>
