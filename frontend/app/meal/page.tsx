...
export default function MealPage() {
  ...
  return <main className="page-shell">
    ...
    <div className="content-stack">
      ...
      {permissions.has("meal.consumption.view") ? <section className="panel"><div className="panel-heading"><div><span className="eyebrow dark">Tüketim geçmişi</span><h2>Son yemek kayıtları</h2><p>Maliyetler, kayıt anındaki fiyat üzerinden gösterilir.</p></div><strong>{consumptions.length}</strong></div><div className="table-wrap responsive-table-wrap" role="region" aria-label="Yemek tüketim geçmişi" tabIndex={0}><table className="data-table responsive-table"><thead><tr><th>Tarih</th><th>Personel</th><th>Kamp</th><th>Öğün</th><th>Adet</th><th>Birim fiyat</th><th>Toplam</th><th>Kaynak</th></tr></thead><tbody>{consumptions.length === 0 ? <tr><td className="empty-row" colSpan={8}>Henüz tüketim kaydı yok.</td></tr> : consumptions.map(x => <tr key={x.id}><td data-label="Tarih">{formatDate(x.consumptionDate)}</td><td data-label="Personel"><strong>{x.employeeName}</strong><small>{x.employeeNo}</small></td><td data-label="Kamp">{x.campCode} · {x.campName}</td><td data-label="Öğün">{x.mealTypeName}</td><td data-label="Adet">{x.quantity.toLocaleString("tr-TR")}</td><td data-label="Birim fiyat">{x.unitPriceSnapshot.toLocaleString("tr-TR", { minimumFractionDigits: 2 })} {x.currencySnapshot}</td><td data-label="Toplam"><strong>{x.totalCostSnapshot.toLocaleString("tr-TR", { minimumFractionDigits: 2 })} {x.currencySnapshot}</strong></td><td data-label="Kaynak"><span className="status-badge">{sourceLabel(x.source)}</span></td></tr>)}</tbody></table></div></section> : null}
    </div>
  </main>;
}
