  ...
  return <main className="page-shell">
    ...
    <div className="content-stack">
      ...
      <section className="panel">
        ...
        <div className="table-wrap"><table className="data-table"><thead><tr><th>Dönem</th><th>Durum</th><th>Satır</th><th>Kabul / Red</th><th>Tutar özeti</th><th>İşlemler</th></tr></thead><tbody>{batches.length === 0 ? <tr><td className="empty-row" colSpan={6}>Henüz ERP aktarım paketi oluşturulmadı.</td></tr> : batches.map(x => { const status = statusOf(x.status); return <tr key={x.id} className={selectedBatch?.id === x.id ? "selected-row" : ""}><td><strong>{new Date(x.fromDate).toLocaleDateString("tr-TR")} – {new Date(x.toDate).toLocaleDateString("tr-TR")}</strong><small>{new Date(x.createdAt).toLocaleString("tr-TR")}</small></td><td><span className={`status-badge ${status.tone}`}>{status.label}</span></td><td>{x.lineCount}</td><td>{x.acceptedLines} / <span className={x.rejectedLines ? "review-count" : ""}>{x.rejectedLines}</span></td><td><small>{totals(x)}</small></td><td><div className="action-row"><button className="secondary-button" type="button" onClick={() => void loadLines(x)}>İncele</button><button className="secondary-button" type="button" onClick={() => void downloadBatch(x)}>CSV indir</button>{x.status === "DRAFT" ? <button className="secondary-button" type="button" onClick={() => void sendBatch(x)}>ERP'ye gönder</button> : null}{x.status === "ACCEPTED" ? <button className="secondary-button button-success" type="button" onClick={() => void closeBatch(x)}>Kapat</button> : null}</div></td></tr>; })}</tbody></table></div>
      </section>
