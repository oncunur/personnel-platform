  ...
  return <main className="page-shell">
    ...
    <div className="content-stack">
      <section className="panel">
        ...
        <div className="table-wrap responsive-table-wrap" role="region" aria-label="ERP hesap eşlemeleri" tabIndex={0}><table className="data-table responsive-table"><thead><tr><th>Maliyet türü</th><th>Hesap kodu</th><th>Karşı hesap</th><th>Durum</th><th>İşlem</th></tr></thead><tbody>{mappings.length === 0 ? <tr><td className="empty-row" colSpan={5}>Bu ERP sistemi için hesap eşlemesi bulunmuyor.</td></tr> : mappings.map(x => <tr key={x.id}><td data-label="Maliyet türü"><strong>{categoryLabels[x.costCategory] ?? x.costCategory}</strong><small>{x.costCategory}</small></td><td data-label="Hesap kodu">{x.accountCode}</td><td data-label="Karşı hesap">{x.counterAccountCode ?? "—"}</td><td data-label="Durum"><span className={`status-badge ${x.isActive ? "success" : ""}`}>{x.isActive ? "Aktif" : "Pasif"}</span></td><td data-label="İşlem"><button className="secondary-button" type="button" disabled={busy} onClick={() => void toggleMapping(x)}>{x.isActive ? "Pasife al" : "Aktife al"}</button></td></tr>)}</tbody></table></div>
        <OperationDisclosure title="Yeni hesap eşlemesi ekle" description="Aktarımdan önce ilgili maliyet türünün hesap kodunu tanımlayın."><form className="inline-form" onSubmit={createMapping}>
      ...
      <section className="panel">
        ...
        <div className="table-wrap responsive-table-wrap" role="region" aria-label="ERP aktarım paketleri" tabIndex={0}><table className="data-table responsive-table"><thead><tr><th>Dönem</th><th>Durum</th><th>Satır</th><th>Kabul / Red</th><th>Tutar özeti</th><th>İşlemler</th></tr></thead><tbody>{batches.length === 0 ? <tr><td className="empty-row" colSpan={6}>Henüz ERP aktarım paketi oluşturulmadı.</td></tr> : batches.map(x => { const status = statusOf(x.status); return <tr key={x.id} className={selectedBatch?.id === x.id ? "selected-row" : ""}><td data-label="Dönem"><strong>{new Date(x.fromDate).toLocaleDateString("tr-TR")} – {new Date(x.toDate).toLocaleDateString("tr-TR")}</strong><small>{new Date(x.createdAt).toLocaleString("tr-TR")}</small></td><td data-label="Durum"><span className={`status-badge ${status.tone}`}>{status.label}</span></td><td data-label="Satır">{x.lineCount}</td><td data-label="Kabul / Red">{x.acceptedLines} / <span className={x.rejectedLines ? "review-count" : ""}>{x.rejectedLines}</span></td><td data-label="Tutar özeti"><small>{totals(x)}</small></td><td data-label="İşlemler"><div className="action-row"><button className="secondary-button" type="button" onClick={() => void loadLines(x)}>İncele</button><button className="secondary-button" type="button" onClick={() => void downloadBatch(x)}>CSV indir</button>{x.status === "DRAFT" ? <button className="secondary-button" type="button" onClick={() => void sendBatch(x)}>ERP'ye gönder</button> : null}{x.status === "ACCEPTED" ? <button className="secondary-button button-success" type="button" onClick={() => void closeBatch(x)}>Kapat</button> : null}</div></td></tr>; })}</tbody></table></div>
        <OperationDisclosure title="Yeni aktarım paketi oluştur" description="Seçilen tarih aralığındaki maliyet kayıtlarını tek pakette toplayın."><form className="inline-form" onSubmit={createBatch}><label className="field-label">Başlangıç tarihi<input type="date" value={from} onChange={e => setFrom(e.target.value)} required /></label><label className="field-label">Bitiş tarihi<input type="date" value={to} onChange={e => setTo(e.target.value)} required /></label><button className="primary-button" disabled={busy || !systemId}><Icon name="plus" size={17}/>Paket oluştur</button></form></OperationDisclosure>
      </section>
