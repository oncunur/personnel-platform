  ...
  return <main className="page-shell">
    ...
    <div className="content-stack">
      {me&&(permissions.has("administration.task.manage")||permissions.has("administration.contract.manage"))?<section className="organization-grid">
        {permissions.has("administration.task.manage")?<article className="panel"><OperationDisclosure title="Yeni idari görev oluştur" description="Tek seferlik veya tekrarlanan bir sorumluluk planlayın."><form className="stack" onSubmit={e => submit(e, "/api/v1/administration/affairs/tasks", fd => { const unit = String(fd.get("unit")); return { companyId, code: fd.get("code"), title: fd.get("title"), description: fd.get("description") || null, responsibleUserId: me.userId, dueDate: fd.get("dueDate"), recurrenceUnit: unit, recurrenceInterval: unit === "NONE" ? 0 : Number(fd.get("interval")), reminderDaysBefore: Number(fd.get("reminder")) }; }, "İdari görev oluşturuldu.")}><label className="field-label">Görev kodu<input name="code" required/></label><label className="field-label">Başlık<input name="title" required/></label><label className="field-label">Açıklama<input name="description"/></label><label className="field-label">İlk son tarih<input name="dueDate" type="date" defaultValue={today()} required/></label><label className="field-label">Tekrar sıklığı<select name="unit" defaultValue="MONTHLY"><option value="NONE">Tek sefer</option><option value="DAILY">Günlük</option><option value="WEEKLY">Haftalık</option><option value="MONTHLY">Aylık</option><option value="YEARLY">Yıllık</option></select></label><label className="field-label">Tekrar aralığı<input name="interval" type="number" min="1" max="365" defaultValue="1"/></label><label className="field-label">Kaç gün önce hatırlatılsın?<input name="reminder" type="number" min="0" max="365" defaultValue="7"/></label><button className="primary-button" disabled={busy || !companyId}>Görevi kaydet</button></form></OperationDisclosure></article>:null}
        {permissions.has("administration.contract.manage")?<article className="panel"><OperationDisclosure title="Yeni kontrat oluştur" description="Taraf, dönem, tutar ve yenileme bilgisini kaydedin."><form className="stack" onSubmit={e => submit(e, "/api/v1/administration/affairs/contracts", fd => { const value = String(fd.get("value") ?? ""); return { companyId, contractNo: fd.get("contractNo"), title: fd.get("title"), counterparty: fd.get("counterparty"), responsibleUserId: me.userId, startDate: fd.get("startDate"), endDate: fd.get("endDate"), reminderDaysBefore: Number(fd.get("reminder")), autoRenewal: fd.get("autoRenewal") === "on", contractValue: value ? Number(value) : null, currency: value ? fd.get("currency") : null, note: fd.get("note") || null }; }, "Kontrat oluşturuldu.")}><label className="field-label">Kontrat numarası<input name="contractNo" required/></label><label className="field-label">Başlık<input name="title" required/></label><label className="field-label">Karşı taraf<input name="counterparty" required/></label><label className="field-label">Başlangıç<input name="startDate" type="date" defaultValue={today()} required/></label><label className="field-label">Bitiş<input name="endDate" type="date" required/></label><label className="field-label">Kaç gün önce hatırlatılsın?<input name="reminder" type="number" min="0" max="730" defaultValue="30"/></label><label className="field-label">Kontrat değeri<input name="value" type="number" min="0" step="0.01"/></label><label className="field-label">Para birimi<input name="currency" defaultValue="TRY" maxLength={3}/></label><label className="check-label"><input name="autoRenewal" type="checkbox"/> Otomatik yenilensin</label><label className="field-label">Not<input name="note"/></label><button className="primary-button" disabled={busy || !companyId}>Kontratı kaydet</button></form></OperationDisclosure></article>:null}
      </section>:null}

      {permissions.has("administration.task.view") ? <section className={`panel attention-panel ${openTasks.length?"":"success"}`}><div className="panel-heading"><div><span className="eyebrow dark">Görev takibi</span><h2>İdari sorumluluklar</h2><p>Açık görevi tamamlayın, duraklatın veya kalıcı olarak kapatın.</p></div><strong>{tasks.length}</strong></div><div className="table-wrap responsive-table-wrap" role="region" aria-label="İdari görevler" tabIndex={0}><table className="data-table responsive-table"><thead><tr><th>Görev</th><th>Sorumlu</th><th>Son tarih</th><th>Tekrar</th><th>Durum</th><th>Tamamlanma</th><th>İşlem</th></tr></thead><tbody>{tasks.length === 0 ? <tr><td className="empty-row" colSpan={7}>Henüz görev yok.</td></tr> : tasks.map(x => <tr key={x.id}><td data-label="Görev"><strong>{x.title}</strong><small>{x.code}</small></td><td data-label="Sorumlu">{x.responsibleUsername}</td><td data-label="Son tarih">{formatDate(x.dueDate)}</td><td data-label="Tekrar">{recurrenceLabel(x.recurrenceUnit)}{x.recurrenceInterval>1?` · Her ${x.recurrenceInterval} dönem`:""}</td><td data-label="Durum"><span className={`status-badge ${x.status==="OPEN"?"success":x.status==="PAUSED"?"warning":""}`}>{taskStatus(x.status)}</span></td><td data-label="Tamamlanma">{x.completionCount}</td><td data-label="İşlem">{permissions.has("administration.task.manage") ? <div className="action-row">{x.status === "OPEN" ? <><button className="secondary-button button-success" onClick={() => void taskAction(x,"complete")}>Tamamla</button><button className="secondary-button" onClick={() => void taskAction(x,"pause")}>Duraklat</button></> : null}{x.status === "PAUSED" ? <button className="secondary-button button-success" onClick={() => void taskAction(x,"resume")}>Devam ettir</button> : null}{["OPEN","PAUSED"].includes(x.status) ? <button className="secondary-button button-danger" onClick={() => void taskAction(x,"close")}>Kapat</button> : null}</div> : "—"}</td></tr>)}</tbody></table></div></section> : null}

      {permissions.has("administration.contract.view") ? <section className={`panel attention-panel ${expiringContracts.length?"warning":"success"}`}><div className="panel-heading"><div><span className="eyebrow dark">Kontrat takibi</span><h2>Kontratlar ve vadeler</h2><p>{expiringContracts.length?`${expiringContracts.length} kontrat için vade dikkati gerekiyor.`:"Yaklaşan veya geçmiş kontrat vadesi yok."}</p></div><strong>{contracts.length}</strong></div><div className="table-wrap responsive-table-wrap" role="region" aria-label="Kontratlar ve vadeler" tabIndex={0}><table className="data-table responsive-table"><thead><tr><th>No</th><th>Kontrat</th><th>Karşı taraf</th><th>Sorumlu</th><th>Bitiş</th><th>Durum</th><th>Değer</th><th>İşlem</th></tr></thead><tbody>{contracts.length === 0 ? <tr><td className="empty-row" colSpan={8}>Henüz kontrat yok.</td></tr> : contracts.map(x => <tr key={x.id}><td data-label="No"><strong>{x.contractNo}</strong></td><td data-label="Kontrat">{x.title}<small>{x.autoRenewal?"Otomatik yenileme":"Manuel yenileme"}</small></td><td data-label="Karşı taraf">{x.counterparty}</td><td data-label="Sorumlu">{x.responsibleUsername}</td><td data-label="Bitiş">{formatDate(x.endDate)}</td><td data-label="Durum"><span className={`status-badge ${x.effectiveStatus==="ACTIVE"?"success":x.effectiveStatus==="EXPIRING"?"warning":x.effectiveStatus==="EXPIRED"?"danger":""}`}>{contractStatus(x.effectiveStatus)}</span></td><td data-label="Değer">{x.contractValue == null ? "—" : `${x.contractValue.toLocaleString("tr-TR",{minimumFractionDigits:2})} ${x.currency}`}</td><td data-label="İşlem">{x.storedStatus === "ACTIVE" && permissions.has("administration.contract.manage") ? <button className="secondary-button button-danger" onClick={() => void closeContract(x)}>Kontratı kapat</button> : "—"}</td></tr>)}</tbody></table></div></section> : null}

      {permissions.has("administration.reminder.view") ? <section className={`panel attention-panel ${importantReminders.length?"warning":""}`}><div className="panel-heading"><div><span className="eyebrow dark">Hatırlatmalar</span><h2>Yaklaşan idari olaylar</h2><p>Araç belgeleri, görevler ve kontratlar için üretilen takip kayıtları.</p></div>{permissions.has("administration.reminder.process") ? <button className="primary-button" disabled={busy} onClick={() => void processReminders()}>Hatırlatmaları tara</button> : null}</div><div className="table-wrap responsive-table-wrap" role="region" aria-label="İdari hatırlatmalar" tabIndex={0}><table className="data-table responsive-table"><thead><tr><th>Oluşma</th><th>Önem</th><th>Tür</th><th>Son tarih</th><th>Mesaj</th></tr></thead><tbody>{reminders.length === 0 ? <tr><td className="empty-row" colSpan={5}>Henüz hatırlatma yok.</td></tr> : reminders.map(x => <tr key={x.id}><td data-label="Oluşma">{new Date(x.createdAt).toLocaleString("tr-TR")}</td><td data-label="Önem"><span className={`status-badge ${["HIGH","CRITICAL"].includes(x.severity)?"danger":x.severity==="MEDIUM"?"warning":""}`}>{x.severity==="CRITICAL"?"Kritik":x.severity==="HIGH"?"Yüksek":x.severity==="MEDIUM"?"Orta":"Bilgi"}</span></td><td data-label="Tür">{x.eventType}</td><td data-label="Son tarih">{formatDate(x.dueDate)}</td><td data-label="Mesaj">{x.message}</td></tr>)}</tbody></table></div></section> : null}
    </div>
    {dialog}
  </main>;
}
