  ...
  return <main className="page-shell">
    ...
    {project ? <>
      ...
      <div className="content-stack">
        ...
        <section className="panel"><div className="panel-heading"><div><span className="eyebrow dark">Maliyet kırılımı</span><h2>Para birimine göre maliyetler</h2><p>Bordro, yemek ve konaklama maliyetlerini ayrı ayrı karşılaştırın.</p></div><strong>{project.costs.length}</strong></div><div className="table-wrap responsive-table-wrap" role="region" aria-label="Proje maliyet kırılımı" tabIndex={0}><table className="data-table responsive-table"><thead><tr><th>Para birimi</th><th>Bordro</th><th>Yemek</th><th>Konaklama</th><th>Toplam</th></tr></thead><tbody>{project.costs.length === 0 ? <tr><td className="empty-row" colSpan={5}>Bu dönemde projeye atanmış maliyet bulunmuyor.</td></tr> : project.costs.map(x => <tr key={x.currency}><td data-label="Para birimi"><strong>{x.currency}</strong></td><td data-label="Bordro">{money(x.payrollCost, x.currency)}</td><td data-label="Yemek">{money(x.mealCost, x.currency)}</td><td data-label="Konaklama">{money(x.accommodationCost, x.currency)}</td><td data-label="Toplam"><strong>{money(x.totalCost, x.currency)}</strong></td></tr>)}</tbody></table></div></section>
