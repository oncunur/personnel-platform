...
export default function ReportsPage() {
  ...
  return <main className="page-shell">
    ...
    <div className="content-stack">
      {permissions.has("reporting.view") ? <section className="panel"><div className="panel-heading"><div><span className="eyebrow dark">Yönetim özeti</span><h2>Proje performansı</h2><p>İnsan gücü, çalışma, tüketim ve maliyet göstergelerini proje bazında karşılaştırın.</p></div><strong>{management.length}</strong></div><div className="table-wrap"><table className="data-table"><thead><tr><th>Proje</th><th>Çalışan / adam-gün</th><th>Saat / fazla mesai</th><th>Yemek / gece</th><th>Toplam maliyet</th><th>Detay</th></tr></thead><tbody>{management.length === 0 ? <tr><td className="empty-row" colSpan={6}>Bu şirket ve dönem için proje verisi yok.</td></tr> : management.map(x => <tr key={x.projectId}><td><strong>{x.projectCode}</strong><small>{x.projectName}</small></td><td>{x.headcount} / {x.manDays.toLocaleString("tr-TR",{maximumFractionDigits:2})}</td><td>{x.workedHours.toLocaleString("tr-TR",{maximumFractionDigits:2})} / {x.approvedOvertimeHours.toLocaleString("tr-TR",{maximumFractionDigits:2})}</td><td>{x.mealQuantity.toLocaleString("tr-TR",{maximumFractionDigits:2})} / {x.accommodationNights.toLocaleString("tr-TR")}</td><td><strong>{costText(x.costs)}</strong></td><td><a className="table-button" href={`/reports/projects/${x.projectId}?from=${from}&to=${to}`}>Proje 360</a></td></tr>)}</tbody></table></div></section> : null}
