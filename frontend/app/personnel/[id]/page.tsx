...
export default function Personel360Page() {
  ...
  return <main className="page-shell">
    ...
    {permissions.has("documents.employee.view") ? <section className="panel audit-panel">
      ...
      {permissions.has("documents.employee.upload") && permissions.has("documents.type.view") ? <div className="form-surface"><div className="form-surface-heading"><div><strong>Yeni belge ekle</strong><span>Belge türünün gerektirdiği tarih, numara ve dosya bilgilerini girin.</span></div></div><form className="inline-form document-form" onSubmit={uploadDocument}>
      ...
      <div className="table-wrap"><table className="data-table"><thead><tr><th>Belge</th><th>No</th><th>Geçerlilik</th><th>Durum</th><th>Dosya</th></tr></thead><tbody>{documents.length === 0 ? <tr><td className="empty-row" colSpan={5}>Bu personel için henüz belge eklenmedi.</td></tr> : documents.map(x => { const status = statusOf(x.status); return <tr key={x.id}><td><strong>{x.documentTypeName}</strong><small>{x.documentTypeCode}</small></td><td>{x.documentNumber ?? "—"}</td><td>{formatDate(x.validFrom ?? x.issueDate)} → {x.validUntil ? formatDate(x.validUntil) : "Süresiz"}</td><td><span className={`status-badge ${status.tone}`}>{status.label}</span></td><td>{x.fileName ? <div className="action-row"><span>{x.fileName}</span>{permissions.has("documents.file.view") ? <button className="table-button document-open" type="button" disabled={busy} onClick={() => void openDocument(x.id)}>Dosyayı aç</button> : null}</div> : "—"}</td></tr>; })}</tbody></table></div>
    </section> : null}
    ...
    {permissions.has("leave.view") || permissions.has("leave.balance.view") ? <section className="panel audit-panel">
      ...
      {permissions.has("leave.view") ? <><div className="table-section-heading"><div className="panel-heading"><div><h3>İzin talepleri</h3></div></div></div><div className="table-wrap"><table className="data-table"><thead><tr><th>İzin türü</th><th>Tarih</th><th>Gün</th><th>Durum</th><th>Açıklama</th></tr></thead><tbody>{leaveRows.length === 0 ? <tr><td className="empty-row" colSpan={5}>Bu personele ait izin talebi bulunmuyor.</td></tr> : leaveRows.map(x => { const status = statusOf(x.status); return <tr key={x.id}><td><strong>{x.leaveTypeName}</strong><small>{x.leaveTypeCode}</small></td><td>{formatDate(x.startDate)} → {formatDate(x.endDate)}<small>{x.startDayPart} / {x.endDayPart}</small></td><td>{x.requestedDays}</td><td><span className={`status-badge ${status.tone}`}>{status.label}</span></td><td>{x.reason ?? "—"}</td></tr>; })}</tbody></table></div></> : null}
      {permissions.has("leave.balance.view") ? <><div className="table-section-heading"><div className="panel-heading"><div><h3>İzin bakiyeleri</h3></div></div></div><div className="table-wrap"><table className="data-table"><thead><tr><th>Bakiye türü</th><th>Dönem</th><th>Hakediş + Devir</th><th>Rezerve</th><th>Kullanılan</th><th>Kullanılabilir</th></tr></thead><tbody>{leaveBalances.length === 0 ? <tr><td className="empty-row" colSpan={6}>Bu personel için izin bakiyesi bulunmuyor.</td></tr> : leaveBalances.map(x => <tr key={x.id}><td><strong>{x.leaveTypeName}</strong><small>{x.leaveTypeCode}</small></td><td>{formatDate(x.periodStart)} → {formatDate(x.periodEnd)}</td><td>{x.entitledDays} + {x.carryOverDays}{x.adjustmentDays !== 0 ? ` (${x.adjustmentDays > 0 ? "+" : ""}${x.adjustmentDays})` : ""}</td><td>{x.reservedDays}</td><td>{x.usedDays}</td><td><strong className="amount-positive">{x.availableDays}</strong></td></tr>)}</tbody></table></div></> : null}
    </section> : null}
    ...
    {permissions.has("workflow.request.view") ? <section className="panel audit-panel">
      ...
      <div className="table-wrap"><table className="data-table"><thead><tr><th>Talep no</th><th>Tür</th><th>Talep eden</th><th>Öncelik</th><th>Durum</th><th>Son tarih</th><th>İşlem</th></tr></thead><tbody>{workflowRequests.length === 0 ? <tr><td className="empty-row" colSpan={7}>Bu personele bağlı talep bulunmuyor.</td></tr> : workflowRequests.map(x => { const status = statusOf(x.status); return <tr key={x.id} className={workflowDetail?.request.id === x.id ? "selected-row" : ""}><td><strong>{x.requestNo}</strong></td><td>{x.requestTypeName}<small>{x.requestTypeCode}</small></td><td>{x.requesterUsername}</td><td>{priorityLabels[x.priority] ?? x.priority}</td><td><span className={`status-badge ${status.tone}`}>{status.label}</span></td><td>{x.dueAt ? new Date(x.dueAt).toLocaleString("tr-TR") : "—"}</td><td><button className="table-button" type="button" disabled={busy} onClick={() => void loadWorkflowDetail(x.id)}>Süreci incele</button></td></tr>; })}</tbody></table></div>
    ...
    {permissions.has("personnel.project.view") ? <section className="panel audit-panel"><div className="panel-heading"><div><span className="eyebrow dark">Proje dağılımı</span><h2>Proje atamaları</h2><p>Personelin çalışma dönemlerini ve proje bazlı kapasite dağılımını izleyin.</p></div><strong>{assignments.length}</strong></div>{permissions.has("personnel.project.assign") ? <div className="form-surface"><div className="form-surface-heading"><div><strong>Yeni proje ataması</strong><span>Toplam kapasite dağılımının aynı dönemde yüzde 100'ü aşmamasına dikkat edin.</span></div></div><form className="inline-form" onSubmit={assignProject}><label className="field-label">Proje<select name="projectId" required><option value="">Proje seçin</option>{projects.map(x => <option key={x.id} value={x.id}>{x.code} · {x.name}</option>)}</select></label><label className="field-label">Maliyet merkezi<select name="costCenterId"><option value="">Merkez seçilmedi</option>{costCenters.map(x => <option key={x.id} value={x.id}>{x.code} · {x.name}</option>)}</select></label><Field name="validFrom" label="Başlangıç" type="date"/><Field name="validUntil" label="Bitiş" type="date"/><Field name="allocationPercent" label="Kapasite oranı (%)" type="number"/><button className="primary-button" disabled={busy}><Icon name="plus" size={17}/>Atama oluştur</button></form></div> : null}<div className="table-wrap"><table className="data-table"><thead><tr><th>Proje</th><th>Maliyet merkezi</th><th>Tarih</th><th>Kapasite</th><th>Durum</th></tr></thead><tbody>{assignments.length === 0 ? <tr><td className="empty-row" colSpan={5}>Bu personelin proje ataması bulunmuyor.</td></tr> : assignments.map(x => { const status = statusOf(x.status); return <tr key={x.id}><td><strong>{lookup(projects, x.projectId)}</strong></td><td>{lookup(costCenters, x.costCenterId)}</td><td>{formatDate(x.validFrom)} → {x.validUntil ? formatDate(x.validUntil) : "Devam ediyor"}</td><td>%{x.allocationPercent}</td><td><span className={`status-badge ${status.tone}`}>{status.label}</span></td></tr>; })}</tbody></table></div></section> : null}
