...
export default function ImportsPage() {
  ...
  return <main className="page-shell">
    ...
    <div className="content-stack">
      ...
      <section className="panel"><div className="panel-heading"><div><span className="eyebrow dark">Geçmiş</span><h2>İçe aktarma işleri</h2><p>Daha önce yüklenen dosyaları ve sonuçlarını inceleyin.</p></div><strong>{jobs.length}</strong></div><div className="table-wrap responsive-table-wrap" role="region" aria-label="İçe aktarma geçmişi" tabIndex={0}><table className="data-table responsive-table"><thead><tr><th>Dosya</th><th>Aktarım türü</th><th>Durum</th><th>Satır</th><th>Başarılı / Hata</th><th>Zaman</th><th>İşlem</th></tr></thead><tbody>{jobs.length === 0 ? <tr><td className="empty-row" colSpan={7}>Henüz bir dosya yüklenmedi.</td></tr> : jobs.map(x => { const status = statusOf(x.status); return <tr key={x.id} className={job?.id === x.id ? "selected-row" : ""}><td data-label="Dosya"><strong>{x.originalFileName}</strong><small>Aktarım kaydı</small></td><td data-label="Aktarım türü">{targetLabels[x.targetType] ?? x.targetType}</td><td data-label="Durum"><span className={`status-badge ${status.tone}`}>{status.label}</span></td><td data-label="Satır">{x.totalRows}</td><td data-label="Başarılı / Hata"><span className="amount-positive">{x.successRows}</span> / <span className={x.errorRows ? "amount-negative" : ""}>{x.errorRows}</span></td><td data-label="Zaman">{dateTime(x.createdAt)}<small>{x.completedAt ? `Tamamlanma: ${dateTime(x.completedAt)}` : ""}</small></td><td data-label="İşlem"><button className="secondary-button" type="button" onClick={() => void openJob(x)}>İncele</button></td></tr>; })}</tbody></table></div></section>
    </div>
    {dialog}
  </main>;
}
