...
export default function ImportsPage() {
  ...
  return <main className="page-shell">
    ...
    <div className="content-stack">
      ...
      <section className="panel"><div className="panel-heading"><div><span className="eyebrow dark">Geçmiş</span><h2>İçe aktarma işleri</h2><p>Daha önce yüklenen dosyaları ve sonuçlarını inceleyin.</p></div><strong>{jobs.length}</strong></div><div className="table-wrap"><table className="data-table selectable-table"><thead><tr><th>Dosya</th><th>Aktarım türü</th><th>Durum</th><th>Satır</th><th>Başarılı / Hata</th><th>Zaman</th><th>İşlem</th></tr></thead><tbody>{jobs.length === 0 ? <tr><td className="empty-row" colSpan={7}>Henüz bir dosya yüklenmedi.</td></tr> : jobs.map(x => { const status = statusOf(x.status); return <tr key={x.id} className={job?.id === x.id ? "selected-row" : ""} onClick={() => void openJob(x)}><td><strong>{x.originalFileName}</strong><small>{x.id.slice(0, 8)}</small></td><td>{targetLabels[x.targetType] ?? x.targetType}</td><td><span className={`status-badge ${status.tone}`}>{status.label}</span></td><td>{x.totalRows}</td><td><span className="amount-positive">{x.successRows}</span> / <span className={x.errorRows ? "amount-negative" : ""}>{x.errorRows}</span></td><td>{dateTime(x.createdAt)}<small>{x.completedAt ? `Tamamlanma: ${dateTime(x.completedAt)}` : ""}</small></td><td><button className="secondary-button" type="button" onClick={event => { event.stopPropagation(); void openJob(x); }}>İncele</button></td></tr>; })}</tbody></table></div></section>
    </div>
  </main>;
}
