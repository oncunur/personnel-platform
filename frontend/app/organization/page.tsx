    ...
    <main className="page-shell">
      ...
      <div className="content-stack">
        ...
        {selectedCompanyId ? <section className="organization-grid">
          <OrgSection eyebrow="Lokasyon" title="Şubeler" description="Şirketin fiziksel veya idari çalışma noktaları." rows={branches.map((x) => <div className="role-row" key={x.id}><strong>{x.name}</strong><span>{x.code} · {x.location || "Lokasyon belirtilmedi"}</span></div>)}>
          ...
          <OrgSection eyebrow="Kadro" title="Pozisyonlar" description={selectedDepartment ? `${selectedDepartment.name} departmanındaki pozisyonlar.` : "Pozisyonları görmek için departman seçin."} rows={positions.map((x) => <div className="role-row" key={x.id}><strong>{x.name}</strong><span>{x.code}</span></div>)}>
            <div className="selection-bar"><label className="field-label">Departman<select value={selectedDepartmentId} onChange={(e) => setSelectedDepartmentId(e.target.value)}><option value="">Departman seçin</option>{departments.map(x => <option key={x.id} value={x.id}>{x.code} · {x.name}</option>)}</select></label><span className="selection-context"><span>Seçili departman</span><strong>{selectedDepartment?.name ?? "—"}</strong></span></div>
...
function OrgForm({ title, description, onSubmit, busy, children }: { title: string; description: string; onSubmit: (e: FormEvent<HTMLFormElement>) => void; busy: boolean; children: React.ReactNode }) { return <section className="panel"><div className="panel-heading"><div><span className="eyebrow dark">Şirket tanımı</span><h2>{title}</h2><p>{description}</p></div></div><div className="form-surface"><form className="inline-form" onSubmit={onSubmit}>{children}<Submit busy={busy}/></form></div></section>; }
function OrgSection({ eyebrow, title, description, rows, children }: { eyebrow: string; title: string; description: string; rows: React.ReactNode[]; children?: React.ReactNode }) { return <article className="panel"><div className="panel-heading"><div><span className="eyebrow dark">{eyebrow}</span><h2>{title}</h2><p>{description}</p></div><strong>{rows.length}</strong></div>{children ? <div className="form-surface">{children}</div> : null}<div className="compact-list">{rows.length ? rows : <p className="muted">Bu bölümde henüz kayıt yok.</p>}</div></article>; }
