...
export default function DashboardPage() {
  ...
  function fetchMe(accessToken: string) {
    return fetch(`${apiBase}/api/v1/auth/me`, { headers: { Authorization: `Bearer ${accessToken}` }, credentials: "include" });
  }

  async function logout() {
    try { await fetch(`${apiBase}/api/v1/auth/logout`, { method: "POST", credentials: "include" }); }
    finally { clearLocalSession(); window.location.replace("/login"); }
  }
  ...
  const canOpenSecurity = me?.permissions.some((permission) => ["system.user.view", "system.role.view", "audit.view"].includes(permission.code)) ?? false;
  const canOpenOrganization = me?.permissions.some((permission) => permission.code.startsWith("organization.")) ?? false;

  return (
    <main className="shell">
      <section className="hero compact">
        <span className="eyebrow">SPRINT 2 · PLATFORM</span>
        <h1>Platform Dashboard</h1>
        <p>{message}</p>
        {me ? <div className="session-summary"><strong>{me.username}</strong><span>{me.email ?? "E-posta tanımlı değil"}</span><span>Security version: {me.securityVersion}</span><span>Roller: {me.roles.map((role) => role.code).join(", ") || "—"}</span><span>Scope: {me.scopes.map((scope) => scope.scopeType).join(", ") || "—"}</span></div> : null}
        <div className="actions action-row">
          {canOpenOrganization ? <a className="primary" href="/organization">Organizasyon</a> : null}
      ...
      <section className="grid" aria-label="Platform durum kartları">
        <article className="card"><span>Aktif</span><h2>Identity / Session</h2></article>
        <article className="card"><span>Aktif</span><h2>Role / Permission / Scope</h2></article>
        <article className="card"><span>Aktif</span><h2>Append-only Audit</h2></article>
        <article className="card"><span>Geliştiriliyor</span><h2>Organization Core</h2></article>
        <article className="card"><span>Sıradaki</span><h2>Personnel Core / Personel 360</h2></article>
      </section>
    </main>
  );
}
