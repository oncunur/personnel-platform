...
export default function LeaveApprovalsPage() {
  ...
  return <main className="page-shell">
    ...
    {(permissions.has("leave.manager.approve") || permissions.has("leave.approve")) && permissions.has("leave.view") ? <section className="panel attention-panel warning">
      ...
      <div className="table-wrap"><table className="data-table"><thead><tr><th>Personel</th><th>İzin</th><th>Tarih</th><th>Adım</th><th>İşlem</th></tr></thead><tbody>{inbox.map(item => <tr key={item.approvalId}><td><strong>{item.employeeName}</strong><small>{item.employeeNo}</small></td><td>{item.leaveTypeName}<small>{item.requestedDays} gün</small></td><td>{formatDate(item.startDate)} → {formatDate(item.endDate)}</td><td><span className="status-badge warning">{item.stepCode === "MANAGER" ? "Yönetici" : "İK nihai"}</span></td><td><div className="action-row"><button className="table-button button-success" disabled={busy || !item.canDecide} onClick={() => void decide(item, true)}>Onayla</button><button className="table-button button-danger" disabled={busy || !item.canDecide} onClick={() => void decide(item, false)}>Reddet</button><button className="table-button" onClick={() => void openWorkflow(item.leaveId)}>Akışı aç</button></div></td></tr>)}{inbox.length === 0 ? <tr><td className="empty-row" colSpan={5}>Karar bekleyen izin bulunmuyor.</td></tr> : null}</tbody></table></div>
    </section> : null}

    {workflow ? <section className="security-grid">
      <article className="panel"><div className="panel-heading"><div><span className="page-eyebrow">Seçili akış</span><h2>{workflow.leave.employeeName}</h2><p>{workflow.leave.leaveTypeName} · {formatDate(workflow.leave.startDate)} → {formatDate(workflow.leave.endDate)} · {workflow.leave.requestedDays} gün</p></div><span className={`status-badge ${statusClass(workflow.leave.status)}`}>{statusLabel(workflow.leave.status)}</span></div><div className="table-wrap"><table className="data-table"><thead><tr><th>Sıra</th><th>Adım</th><th>Atanan</th><th>Durum</th><th>Karar</th></tr></thead><tbody>{workflow.steps.map(step => <tr key={step.id}><td>{step.stepOrder}</td><td>{step.stepCode === "MANAGER" ? "Yönetici" : "İK nihai"}</td><td>{step.assignedUsername ?? step.approverEmployeeName ?? "Rol bazlı"}</td><td><span className={`status-badge ${statusClass(step.status)}`}>{statusLabel(step.status)}</span></td><td>{step.decidedByUsername ?? "—"}<small>{step.decisionNote ?? ""}</small></td></tr>)}</tbody></table></div></article>
      <article className="panel"><div className="panel-heading"><div><span className="page-eyebrow">İşlem izi</span><h2>Onay geçmişi</h2><p>Akışta gerçekleşen tüm karar ve durum değişiklikleri.</p></div><strong>{workflow.history.length}</strong></div><div className="table-wrap"><table className="data-table"><thead><tr><th>Zaman</th><th>Olay</th><th>Durum</th><th>Kullanıcı</th></tr></thead><tbody>{workflow.history.map(h => <tr key={h.id}><td>{new Date(h.occurredAt).toLocaleString("tr-TR")}</td><td>{historyLabel(h.action)}<small>{h.stepCode ?? ""}</small></td><td>{statusLabel(h.fromStatus ?? "—")} → {statusLabel(h.toStatus ?? "—")}<small>{h.note ?? ""}</small></td><td>{h.actorUsername ?? "Sistem"}</td></tr>)}</tbody></table></div></article>
    </section> : null}
    ...
    {permissions.has("leave.approver.manage") ? <section className="panel">
      ...
      {permissions.has("system.user.view") && permissions.has("personnel.view") ? <div className="form-surface"><div className="form-surface-heading"><div><strong>Yeni eşleme</strong><span>Bir kullanıcı ve personel yalnız bir aktif eşleşmede yer alabilir.</span></div></div><form className="inline-form" onSubmit={setLink}><label className="field-label">Sistem kullanıcısı<select name="userId" required><option value="">Seçin</option>{users.map(x => <option key={x.id} value={x.id}>{x.username}{x.email ? ` · ${x.email}` : ""}</option>)}</select></label><label className="field-label">Personel kimliği<select name="employeeId" required><option value="">Seçin</option>{employees.map(x => <option key={x.id} value={x.id}>{x.employeeNo} · {x.firstName} {x.lastName}</option>)}</select></label><button className="primary-button" disabled={busy}>Eşleştir</button></form></div> : <div className="notice">Eşleme düzenlemek için kullanıcı ve personel görüntüleme yetkileri gerekir.</div>}
