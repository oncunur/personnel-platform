...
export default function AttendancePage() {
  ...
  return <main className="page-shell">
    ...
    {permissions.has("attendance.calendar.view") ? <section className="panel">
      ...
      {permissions.has("attendance.calendar.manage") ? <OperationDisclosure title="Yeni çalışma takvimi" description="Şirket için kullanılabilir bir takvim tanımlayın."><form className="inline-form" onSubmit={createCalendar}><label className="field-label">Kod<input name="code" required maxLength={80}/></label><label className="field-label">Ad<input name="name" required maxLength={150}/></label><label className="check-label"><input name="isDefault" type="checkbox"/> Varsayılan takvim</label><button className="primary-button" disabled={busy || !companyId}>{busy ? "Kaydediliyor…" : "Takvim oluştur"}</button></form></OperationDisclosure> : null}
    ...
    {permissions.has("attendance.shift.view") ? <section className="panel">
      ...
      {permissions.has("attendance.shift.manage") ? <OperationDisclosure title="Yeni vardiya tanımla" description="Bitiş saati başlangıçtan küçükse vardiya ertesi güne taşar."><form className="inline-form" onSubmit={createShift}><label className="field-label">Kod<input name="code" required maxLength={80}/></label><label className="field-label">Ad<input name="name" required maxLength={150}/></label><label className="field-label">Başlangıç<input name="startTime" type="time" required/></label><label className="field-label">Bitiş<input name="endTime" type="time" required/></label><label className="field-label">Mola (dk)<input name="breakMinutes" type="number" min={0} defaultValue={60}/></label><label className="field-label">Giriş toleransı<input name="graceInMinutes" type="number" min={0} max={240} defaultValue={0}/></label><label className="field-label">Çıkış toleransı<input name="graceOutMinutes" type="number" min={0} max={240} defaultValue={0}/></label><button className="primary-button" disabled={busy || !companyId}>Vardiya oluştur</button></form></OperationDisclosure> : null}
      <div className="table-wrap responsive-table-wrap" role="region" aria-label="Vardiya tanımları" tabIndex={0}><table className="data-table responsive-table"><thead><tr><th>Vardiya</th><th>Saat</th><th>Mola</th><th>Planlanan</th><th>Tip</th></tr></thead><tbody>{shifts.map(x => <tr key={x.id}><td data-label="Vardiya"><strong>{x.name}</strong><small>{x.code}</small></td><td data-label="Saat">{x.startTime} → {x.endTime}</td><td data-label="Mola">{x.breakMinutes} dk</td><td data-label="Planlanan">{x.plannedMinutes} dk</td><td data-label="Tip"><span className={`status-badge ${x.crossesMidnight ? "warning" : "success"}`}>{x.crossesMidnight ? "Gece / ertesi gün" : "Aynı gün"}</span></td></tr>)}{shifts.length === 0 ? <tr><td className="empty-row" colSpan={5}>Bu şirket için vardiya tanımı bulunmuyor.</td></tr> : null}</tbody></table></div>
    </section> : null}
    ...
    {permissions.has("attendance.assignment.view") ? <section className="panel">
      ...
      {permissions.has("attendance.assignment.manage") && employeeId ? <OperationDisclosure title="Yeni vardiya ataması" description="Geçerlilik tarihleri personelin vardiya geçmişini belirler."><form className="inline-form" onSubmit={assignShift}><label className="field-label">Vardiya<select name="shiftId" required><option value="">Seçin</option>{shifts.filter(x => x.isActive).map(x => <option key={x.id} value={x.id}>{x.code} · {x.name}</option>)}</select></label><label className="field-label">Takvim<select name="workCalendarId" required><option value="">Seçin</option>{calendars.filter(x => x.isActive).map(x => <option key={x.id} value={x.id}>{x.code} · {x.name}</option>)}</select></label><label className="field-label">Başlangıç<input name="validFrom" type="date" required/></label><label className="field-label">Bitiş<input name="validUntil" type="date"/></label><label className="field-label">Not<input name="note" maxLength={1000}/></label><button className="primary-button" disabled={busy}>Vardiya ata</button></form></OperationDisclosure> : null}
      <div className="table-wrap responsive-table-wrap" role="region" aria-label="Personel vardiya atamaları" tabIndex={0}><table className="data-table responsive-table"><thead><tr><th>Vardiya</th><th>Takvim</th><th>Geçerlilik</th><th>Saat</th><th>Planlanan</th></tr></thead><tbody>{assignments.map(x => <tr key={x.id}><td data-label="Vardiya"><strong>{x.shiftName}</strong><small>{x.shiftCode}{x.crossesMidnight ? " · Gece" : ""}</small></td><td data-label="Takvim">{x.calendarName}<small>{x.calendarCode}</small></td><td data-label="Geçerlilik">{formatDate(x.validFrom)} → {x.validUntil ? formatDate(x.validUntil) : "Devam ediyor"}</td><td data-label="Saat">{x.startTime} → {x.endTime}</td><td data-label="Planlanan">{x.plannedMinutes} dk</td></tr>)}{assignments.length === 0 ? <tr><td className="empty-row" colSpan={5}>{employeeId ? "Vardiya ataması bulunmuyor." : "Atamaları görüntülemek için personel seçin."}</td></tr> : null}</tbody></table></div>
    </section> : null}
    </div>
  </main>;
}
