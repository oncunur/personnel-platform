...
export default function SecurityPage() {
  ...
  return (
    <main className="page-shell">
      ...
      <section className="panel session-panel"><div className="workspace-copy"><span className="eyebrow dark">Mevcut oturum</span><h2>{me?.username ?? "Kullanıcı yükleniyor"}</h2><p>{me?.email ?? "E-posta bilgisi yok"} · Oturum güvenlik sürümü {me?.securityVersion ?? "—"}</p></div><div><span className="session-label">Roller</span><div className="role-chips">{me?.roles.length ? me.roles.map(role => <span className="role-chip" key={role.id}>{role.name}</span>) : <span className="role-chip">Rol yok</span>}</div></div><div><span className="session-label">Erişim kapsamı</span><div className="role-chips">{me?.scopes.length ? me.scopes.map((scope,index) => <span className="role-chip" title={scope.scopeId ?? "Tüm kayıtlar"} key={`${scope.scopeType}-${scope.scopeId}-${index}`}>{scopeLabels[scope.scopeType] ?? "Özel kapsam"}{scope.scopeId ? " · Seçili kayıtlar" : ""}</span>) : <span className="role-chip">Kapsam yok</span>}</div></div></section>

      <div className="content-stack">
        <section className="security-grid">
          <article className="panel"><div className="panel-heading"><div><span className="eyebrow dark">Kullanıcı erişimi</span><h2>Kullanıcılar</h2><p>Hesap durumunu ve son giriş bilgisini yönetin.</p></div><strong>{users.length}</strong></div>{permissionCodes.has("system.user.view") ? <div className="table-wrap responsive-table-wrap" role="region" aria-label="Sistem kullanıcıları" tabIndex={0}><table className="data-table responsive-table"><thead><tr><th>Kullanıcı</th><th>Durum</th><th>Güvenlik sürümü</th><th>Son giriş</th><th>İşlem</th></tr></thead><tbody>{users.length?users.map((user) => <tr key={user.id}><td data-label="Kullanıcı"><strong>{user.username}</strong><small>{user.email ?? "E-posta yok"}</small></td><td data-label="Durum"><span className={`status-badge ${user.isActive ? "success" : "danger"}`}>{user.isActive ? "Aktif" : "Pasif"}</span></td><td data-label="Güvenlik sürümü">{user.securityVersion}</td><td data-label="Son giriş">{user.lastLoginAt ? new Date(user.lastLoginAt).toLocaleString("tr-TR") : "—"}</td><td data-label="İşlem">{permissionCodes.has("system.user.manage") && user.id !== me?.userId ? <button className={`table-button ${user.isActive?"button-danger":"button-success"}`} disabled={busyUserId === user.id} type="button" onClick={() => void setUserActive(user, !user.isActive)}>{user.isActive ? "Pasife al" : "Aktifleştir"}</button> : user.id===me?.userId?"Mevcut kullanıcı":"—"}</td></tr>):<tr><td className="empty-row" colSpan={5}>Kullanıcı kaydı yok.</td></tr>}</tbody></table></div> : <p className="notice">Kullanıcı listesini görüntüleme yetkiniz yok.</p>}</article>
