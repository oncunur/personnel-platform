...
export default function WorkflowPage() {
  ...
  return <main className="page-shell">
    ...
    <div className="content-stack">
      {permissions.has("workflow.request_type.manage") ? <section className="panel"><div className="panel-heading"><div><span className="eyebrow dark">Talep türleri</span><h2>Tür ve hizmet süresi tanımları</h2><p>Talep türlerini ve sonuçlandırma hedeflerini yönetin; onay akışını işlem düğmesiyle seçin.</p></div><strong>{types.length}</strong></div><div className="table-wrap responsive-table-wrap" role="region" aria-label="İş akışı talep türleri" tabIndex={0}><table className="data-table responsive-table"><thead><tr><th>Kod</th><th>Ad</th><th>Hedef süre</th><th>Zorunlu alanlar</th><th>Durum</th><th>İşlem</th></tr></thead><tbody>{types.length?types.map(x => <tr key={x.id} className={x.id===selectedTypeId?"selected-row":""}><td data-label="Kod"><strong>{x.code}</strong></td><td data-label="Ad">{x.name}</td><td data-label="Hedef süre">{x.slaMinutes.toLocaleString("tr-TR")} dakika</td><td data-label="Zorunlu alanlar"><code>{x.requiredFieldsJson}</code></td><td data-label="Durum"><span className={`status-badge ${x.isActive?"success":"danger"}`}>{x.isActive?"Aktif":"Pasif"}</span></td><td data-label="İşlem"><button className="table-button" type="button" onClick={() => setSelectedTypeId(x.id)}>Akışı düzenle</button></td></tr>):<tr><td className="empty-row" colSpan={6}>Henüz talep türü yok.</td></tr>}</tbody></table></div><OperationDisclosure title="Yeni talep türü oluştur" description="Tür kodunu, hedef süreyi ve zorunlu alanları tanımlayın."><form className="inline-form" onSubmit={createType}><label className="field-label">Tür kodu<input name="code" required/></label><label className="field-label">Talep türü adı<input name="name" required/></label><label className="field-label">Açıklama<input name="description"/></label><label className="field-label">Hedef süre (dakika)<input name="slaMinutes" type="number" min="1" defaultValue="1440" required/></label><label className="field-label">Zorunlu alan tanımı<input name="requiredFieldsJson" defaultValue='["reason"]' required/></label><button className="primary-button" disabled={busy || !companyId}>Türü kaydet</button></form></OperationDisclosure></section> : null}
      ...
      {permissions.has("workflow.request.create") ? <section className="panel"><OperationDisclosure title="Yeni talep taslağı oluştur" description="Taslağı kaydettikten sonra ayrıntı bölümünden onaya gönderebilirsiniz."><form className="inline-form" onSubmit={createRequest}><label className="field-label">Talep türü<select name="requestTypeId" required><option value="">Tür seçin</option>{types.filter(x => x.isActive).map(x => <option key={x.id} value={x.id}>{x.code} · {x.name}</option>)}</select></label><label className="field-label">İlgili personel<select name="employeeId"><option value="">Personel bağlantısı yok</option>{companyEmployees.map(x => <option key={x.id} value={x.id}>{x.employeeNo} · {x.firstName} {x.lastName}</option>)}</select></label><label className="field-label">Öncelik<select name="priority" defaultValue="NORMAL"><option value="INFO">Bilgi</option><option value="NORMAL">Normal</option><option value="IMPORTANT">Önemli</option><option value="CRITICAL">Kritik</option></select></label><label className="field-label">Talep verisi<input name="payload" defaultValue='{"reason":""}' required/></label><button className="primary-button" disabled={busy || !companyId}>Taslağı kaydet</button></form></OperationDisclosure></section> : null}

      {permissions.has("workflow.request.view") ? <section className="panel"><div className="panel-heading"><div><span className="eyebrow dark">Talep listesi</span><h2>Güncel ve geçmiş talepler</h2><p>Detay, onay ve zaman çizelgesini işlem düğmesiyle açın.</p></div><strong>{requests.length}</strong></div><div className="table-wrap responsive-table-wrap" role="region" aria-label="Güncel ve geçmiş iş akışı talepleri" tabIndex={0}><table className="data-table responsive-table"><thead><tr><th>No</th><th>Tür</th><th>Personel</th><th>Öncelik</th><th>Durum</th><th>Adım</th><th>Hedef tarih</th><th>İşlem</th></tr></thead><tbody>{requests.length?requests.map(x => <tr key={x.id} className={detail?.request.id===x.id?"selected-row":""}><td data-label="No"><strong>{x.requestNo}</strong></td><td data-label="Tür">{x.requestTypeCode}<small>{x.requestTypeName}</small></td><td data-label="Personel">{x.employeeNo ? `${x.employeeNo} · ${x.employeeName}` : "—"}</td><td data-label="Öncelik"><span className={`status-badge ${x.priority==="CRITICAL"?"danger":x.priority==="IMPORTANT"?"warning":""}`}>{priorityLabel(x.priority)}</span></td><td data-label="Durum"><span className={`status-badge ${x.status==="APPROVED"?"success":x.status==="REJECTED"?"danger":x.status==="IN_APPROVAL"?"warning":""}`}>{requestStatus(x.status)}</span></td><td data-label="Adım">{x.currentStepOrder || "—"}</td><td data-label="Hedef tarih">{x.dueAt ? new Date(x.dueAt).toLocaleString("tr-TR") : "—"}</td><td data-label="İşlem"><button className="table-button" type="button" onClick={() => void openRequest(x.id)}>İncele</button></td></tr>):<tr><td className="empty-row" colSpan={8}>Henüz talep yok.</td></tr>}</tbody></table></div></section> : null}
