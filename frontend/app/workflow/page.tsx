...
export default function WorkflowPage() {
  ...
  return <main className="page-shell">
    ...
    <div className="content-stack">
      {permissions.has("workflow.request_type.manage") ? <section className="panel"><div className="panel-heading"><div><span className="eyebrow dark">Talep türleri</span><h2>Tür ve hizmet süresi tanımları</h2><p>Yeni talep türünü ve sonuçlandırma hedefini oluşturun; onay akışını işlem düğmesiyle seçin.</p></div><strong>{types.length}</strong></div><div className="form-surface"><form className="inline-form" onSubmit={createType}><label className="field-label">Tür kodu<input name="code" required/></label><label className="field-label">Talep türü adı<input name="name" required/></label><label className="field-label">Açıklama<input name="description"/></label><label className="field-label">Hedef süre (dakika)<input name="slaMinutes" type="number" min="1" defaultValue="1440" required/></label><label className="field-label">Zorunlu alan tanımı<input name="requiredFieldsJson" defaultValue='["reason"]' required/></label><button className="primary-button" disabled={busy || !companyId}>Türü kaydet</button></form></div><div className="table-wrap"><table className="data-table"><thead><tr><th>Kod</th><th>Ad</th><th>Hedef süre</th><th>Zorunlu alanlar</th><th>Durum</th><th>İşlem</th></tr></thead><tbody>{types.length?types.map(x => <tr key={x.id} className={x.id===selectedTypeId?"selected-row":""}><td><strong>{x.code}</strong></td><td>{x.name}</td><td>{x.slaMinutes.toLocaleString("tr-TR")} dakika</td><td><code>{x.requiredFieldsJson}</code></td><td><span className={`status-badge ${x.isActive?"success":"danger"}`}>{x.isActive?"Aktif":"Pasif"}</span></td><td><button className="table-button" type="button" onClick={() => setSelectedTypeId(x.id)}>Akışı düzenle</button></td></tr>):<tr><td className="empty-row" colSpan={6}>Henüz talep türü yok.</td></tr>}</tbody></table></div></section> : null}
      ...
      {permissions.has("workflow.request.view") ? <section className="panel"><div className="panel-heading"><div><span className="eyebrow dark">Talep listesi</span><h2>Güncel ve geçmiş talepler</h2><p>Detay, onay ve zaman çizelgesini işlem düğmesiyle açın.</p></div><strong>{requests.length}</strong></div><div className="table-wrap"><table className="data-table"><thead><tr><th>No</th><th>Tür</th><th>Personel</th><th>Öncelik</th><th>Durum</th><th>Adım</th><th>Hedef tarih</th><th>İşlem</th></tr></thead><tbody>{requests.length?requests.map(x => <tr key={x.id} className={detail?.request.id===x.id?"selected-row":""}><td><strong>{x.requestNo}</strong></td><td>{x.requestTypeCode}<small>{x.requestTypeName}</small></td><td>{x.employeeNo ? `${x.employeeNo} · ${x.employeeName}` : "—"}</td><td><span className={`status-badge ${x.priority==="CRITICAL"?"danger":x.priority==="IMPORTANT"?"warning":""}`}>{priorityLabel(x.priority)}</span></td><td><span className={`status-badge ${x.status==="APPROVED"?"success":x.status==="REJECTED"?"danger":x.status==="IN_APPROVAL"?"warning":""}`}>{requestStatus(x.status)}</span></td><td>{x.currentStepOrder || "—"}</td><td>{x.dueAt ? new Date(x.dueAt).toLocaleString("tr-TR") : "—"}</td><td><button className="table-button" type="button" onClick={() => void openRequest(x.id)}>İncele</button></td></tr>):<tr><td className="empty-row" colSpan={8}>Henüz talep yok.</td></tr>}</tbody></table></div></section> : null}
